...
class shufflenet_v2(object):
    ...
    def _build_model(self):
        ...
        self.label_y = tf.one_hot(self.y, self.num_classes)
...
with graph_s.as_default():
    ...
    total_loss = tf.add_n([pred_loss] + regularization_losses, name='total_loss')
    
    #regular_train_op = tf.train.MomentumOptimizer(learning_rate, 0.9).minimize(total_loss)
    regular_train_op = tf.train.RMSPropOptimizer(learning_rate, decay=0.9, momentum=0.9, epsilon=1.0).minimize(total_loss)
    scale_inner = model_s.scale_inner
    
    # Evaluation
    correct_label_pred = tf.equal(tf.argmax(model_s.classify_labels, 1), tf.argmax(pred, 1))
...
def train_and_evaluate(training_mode, graph, model,logdir, num_steps=8600,  verbose=False):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        domain_labels = np.vstack([np.tile([1., 0.], [FLAGS.batch_size, 1]),
                                   np.tile([0., 1.], [FLAGS.batch_size, 1])])
        ...
        for i in range(num_steps):
            ...
            source_lr = _learning_rate_fn(0.001, i)
            # Training step
            if training_mode == 'dann':
                _, X0, y0 = dataset.next_batch_source()
                _, X1, y1 = dataset.next_batch_target()
                ...
                if i % 10 == 0:
                    logging.info('iter:{} loss: {:.4}  d_acc: {:.4}  p_acc: {:.4}  p: {:.4}  l: {:.4}  lr: {:.4}'.format(i,
                            batch_loss, d_acc, p_acc, p, l, lr))
            elif training_mode == 'source':
                _, X, y = dataset.next_batch_source()
                _, batch_loss, ploss, p_acc, scale = sess.run([regular_train_op, total_loss, pred_loss, label_acc, scale_inner],
                                     feed_dict={model.X: X, model.y: y, model.train: True,
