...
flags = tf.app.flags
flags.DEFINE_string('checkpoint_path', './trained_models/source_train_20190603_225957/model_iter_58000.ckpt',
                    'The path to a checkpoint from which to fine-tune.')
flags.DEFINE_integer('batch_size', 16, 'Batch size.')
flags.DEFINE_integer('num_steps', 1000000, 'Batch size.')
flags.DEFINE_integer('base_lr', 0.0001, 'Batch size.')
flags.DEFINE_string('train_types', 'dann_train', 'train types')
...
class shufflenet_v2(object):
    ...
    def _build_model(self):
        ...
        self.num_source_batches = dataset._num_train_source_batch
        ...
        with tf.variable_scope('domain_predictor'):
            ...
            d_h_fc0 = slim.fully_connected(feat, 100, activation_fn = tf.nn.relu, scope = 'fc1')
            d_logits = slim.fully_connected(d_h_fc0, 2, activation_fn = None, scope = 'fc2')
...
def train_and_evaluate(training_mode, graph, model,logdir, num_steps=8600,  verbose=False):
    ...
    logging.info('num_classes of source: {}'.format(dataset._num_class_source))
    logging.info('num_classes of target: {}'.format(dataset._num_class_target))
    ...
    with tf.Session(graph=graph) as sess:
        ...
        for i in range(num_steps):
            ...
            source_lr = _learning_rate_fn(FLAGS.base_lr, i)
            # Training step
            if training_mode == 'dann':
                ...
                print ('cur source, target iter:', dataset.num_train_source_iter(), dataset.num_train_target_iter())
                ...
                old_time = cur_time
                logging.info('iter:{}/{} Time: {:.4f}s loss: {:.4f} ploss: {:.4f} d_acc: {:.4f}  p_acc: {:.4f}  p: {:.4f}  l: {:.4f}  lr: {:.4f}  scale:{:.4f}'.format(i,
                        model.num_source_batches, elapsed, batch_loss, ploss, d_acc, p_acc, p, l, source_lr, scale))
                ...
                elapsed = cur_time - old_time
                old_time = cur_time
                logging.info('iter:{}/{} Time: {:.4f}s loss: {:.4f} ploss: {:.4f}  p: {:.4f}  l: {:.4f}  lr: {:.4f}  scale:{:.4f}'.format(i,
                        model.num_source_batches, elapsed, batch_loss, ploss, p, l, source_lr, scale))
                if i == 0 or (i + 1) % 1000 == 0:
...
train_and_evaluate('dann', graph_s, model_s, sub_save_dir, FLAGS.num_steps)
