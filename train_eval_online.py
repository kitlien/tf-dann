...
flags = tf.app.flags
flags.DEFINE_string('checkpoint_path', './trained_models/prime_source_train_20190701_052013/model_iter_6000.ckpt',
                    'The path to a checkpoint from which to fine-tune.')
flags.DEFINE_string('crop_type', '224x224_with_coord',
                    'The path to a checkpoint from which to fine-tune.')
flags.DEFINE_string('train_types', 'prime_source_train', 'train types')
flags.DEFINE_string('file_names', './primesense/old_new_same_id_in_source.txt',
                    'The path to a checkpoint from which to fine-tune.')
...
flags.DEFINE_integer('max_nrof_epochs',100,'epoch size.')
flags.DEFINE_integer('base_lr',0.0001,'base learning rate.')
flags.DEFINE_integer('image_height',256,'image size.')
flags.DEFINE_integer('image_width',256,'image size.')
...
def normalized_linear_layer(inputs, 
    ...
    with tf.variable_scope(name, 'normalized_linear_layer', [inputs], reuse=reuse):
        ...
        return outputs
def build_model_source(image_batch, labels_batch, learning_rate_placeholder, global_step):
    embedding_size = 128
    scale = 4.5
    weight_decay = 0.0005
...
def train_and_evaluate(training_mode, graph, model, logdir, num_steps=8600,  verbose=False):
    ...
    with tf.Graph().as_default():
        ...
        for _ in range(num_dap_threads):
            ...
            image = dap_func(decoded[:-1], augment = True)
        ...
        with sess.as_default():
            ...
            for epoch in range(1, FLAGS.max_nrof_epochs):
                ...
                for i in range(num_batches):
                    ...
                    _, batch_loss, ploss, scale, step = sess.run([regular_train_op, regular_loss, pred_loss, scale_inner, global_step],
                                         feed_dict = feed_dict)
