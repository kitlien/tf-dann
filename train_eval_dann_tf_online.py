...
flags.DEFINE_string('crop_type', '224x224_with_coord',
                    'The path to a checkpoint from which to fine-tune.')
flags.DEFINE_string('train_types', 'prime_dann_train', 'train types')
flags.DEFINE_string('file_names', 'primesense/prime_source_target_pair.txt',
                    'The path to a checkpoint from which to fine-tune.')
...
flags.DEFINE_integer('max_nrof_epochs',20,'epoch size.')
...
def build_model_source(source_image_batch, target_image_batch,\
    ...
    with tf.variable_scope('domain_predictor'):
        ...
        d_h_fc0 = slim.fully_connected(feat, 2048, activation_fn = tf.nn.relu, scope = 'fc1')
        d_h_fc1 = slim.fully_connected(d_h_fc0, 2048, activation_fn = tf.nn.relu, scope = 'fc2')
...
def train_and_evaluate(training_mode, graph, model, logdir, num_steps=8600,  verbose=False):
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            saver = tf.train.Saver(default_to_restore, max_to_keep=30)
